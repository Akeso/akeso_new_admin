<template>
  <div>
        ...
        <div style="padding: 20px 0; margin: 0 20px;">
          ...
          <div>
            ...
            <el-row style="margin:0 10px;">
              <el-col :span="16">
                <MonthHealthItem :text="textHealthIndex" :hours="healthIndexArray" :times="timeArray"/>
    ...
    <factors-tab
      :out-time="monthData.outTime"
      :step-count="monthData.stepCount"
      :out-time-lux="monthData.outTime"
      :nearwork-day="monthData.nearworkDay"
      :nearwork-burden-day="monthData.nearworkBurdenDay"
      :bad-posture-times="monthData.badPostureTimes"
      :reach-days="reach_days"/>
  </div>
</template>
  ...
  methods: {
    getData() {
      fetchMonthly({child_id: this.userId, date: this.selectDate}).then(response => {
        ...
        this.reach_days = response.data.reachDays
        console.log('reach_days => ', this.reach_days)
      })
    },
    changeDate() {
      this.getData()
