  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>{{ generateShow('route.by_child') }}</span>
      </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          min-width="60">
          <template slot-scope="scope">
            {{ scope.row.child_gender | genderFilter }}
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            {{ scope.row.follow_state | followStateFilter }}
          </template>
        </el-table-column>
        <el-table-column
          :label="generateShow('common.operate')"
          min-width="120">
          <template v-if="scope.row.follow_state === 'following'" slot-scope="scope">
...
<script>
import { fetchList, updateState } from '@/api/examination_follow'
export default {
  filters: {
